...
# Two rows sharing PO+date+time+carrier (two separate trucks, see Round 30's
# revert of the Round 28 lots-merge in importPlan.js) against a Supabase
# project that hasn't run the "truck_label" column migration yet -- the
# insert should retry without "truck_label" (keeping "raw") and still save
# both trucks, just without their "<PO> - Truck N" display label.
STUB_XLSX = """
window.XLSX = {
...
async def handle_supabase(route, request):
    ...
    if "/rest/v1/trucks" in url and method == "POST":
        ...
        # Simulate a project where "truck_label" doesn't exist yet, but "raw" does.
...
async def main():
    async with async_playwright() as p:
        ...
        print("Toast after import (truck_label column missing):", toast)

        print("POST attempts:", len(POST_ATTEMPTS))
        assert len(POST_ATTEMPTS) == 2, "expected a rejected attempt with truck_label + a retry without it"
        assert "truck_label" in POST_ATTEMPTS[0][0]
        assert "raw" in POST_ATTEMPTS[0][0], "raw should still be attempted (only truck_label is missing here)"
        assert "truck_label" not in POST_ATTEMPTS[1][0]
        assert "raw" in POST_ATTEMPTS[1][0], "raw must survive the truck_label-only fallback"
        assert len(TRUCKS) == 2, "both rows must still import as separate trucks even when truck_label can't be saved"
        assert TRUCKS[0]["po_no"] == "PO-88002" and TRUCKS[1]["po_no"] == "PO-88002"
        assert TRUCKS[0]["details"] == "[RM] Sacks" and TRUCKS[1]["details"] == "[RM] Crates"
        assert "truck_label" not in TRUCKS[0] and "truck_label" not in TRUCKS[1]
        assert ("แจ้งทีม ISD" in (toast or "")) or ("ask ISD" in (toast or "")), "toast should mention the truck_label migration, not just raw"

        await browser.close()
        print("TRUCK-LABEL-MISSING FALLBACK TEST PASSED")
